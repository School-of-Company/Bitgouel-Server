...
class FaqServiceImplTest : BehaviorSpec({
    ...
    Given("FAQ 가 주어질 때") {
        ...
        val response = fixture<FaqDetailsResponse> {
            property(FaqDetailsResponse::id) { faqId }
            property(FaqDetailsResponse::question) { faq.question }
            property(FaqDetailsResponse::answer) { faq.answer }
        }

        every { faqRepository.findByIdOrNull(faqId) } returns faq

        When("FAQ 전체 조회 요청을 하면") {
            val result = faqServiceImpl.queryFaqDetails(faqId)
